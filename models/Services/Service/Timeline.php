...
class Timeline extends Service
{
    ...
    {
        return TimelineEntity::findFirstById($id) ?: null;
    }
    public function findFirstByUuid($uuid)
    {
        $e = TimelineEntity::find(["uuid = :uuid:", "bind" => ["uuid" => $uuid]]);
        return $e->getFirst();
    }
    /**
    ...
    public function getFirstByModel($model){
        // A raw SQL statement
        $sql = "SELECT * FROM viettel1_timeline WHERE data->>'model' = '{$model}' LIMIT 1 OFFSET 0";
        // Base model
...
//        }
    ...
    public static function cache($filter){
        $module= @$filter['module'] ?: 'cms';
        $type = @$filter['type'] ?: 'catalog';
        $model = @$filter['model'] ?: null;
        $name = @$filter['name'] ?: 'Timeline';
...
//        $sql = "SELECT *, count(*) OVER() as total_count  FROM hse_qr_query(?,?,?,?,?,?,?)";
        $sql = "SELECT * FROM core_Timeline_filter_with_paging(?,?,?,?,?,?,?,?)";
...
//            $m->getReadConnection()->query($sql, array($lang, $order, $page_size, $page_number))
    ...
    public function findBySlug($option){
        ...
        $m = new TimelineEntity();
//        $sql = "SELECT *, count(*) OVER() as total_count  FROM hse_qr_query(?,?,?,?,?,?,?)";
        $sql = "SELECT * FROM core_Timeline_by_slug(?,?,?,?,?,?,?,?,?,?)";
        $resultset = new Resultset(
            null,
            $m,
